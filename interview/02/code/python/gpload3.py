...
try:
    ...
    sys.stderr.write(
        "gpload needs envyaml.  You can get it from https://github.com/thesimj/envyaml.\n"
    )
    sys.exit(2)
...
valid_tokens = {
    ...
    # new
    "create_output_table": {"parse_children": False, "parent": "preload"},
...
class gpload:
    ...
    def __init__(self, argv):
            ...
            try:
                try:
                    ...
                    if argv[0] == "--gpfdist_timeout":
                        ...
                        sys.stderr.write(
                            "gpload version 6.12.1 build commit:7ec4678f29dd922d7d44501f5fc344b5d0d4d49f Open Source\n"
                        )
                        sys.exit(0)
                    elif argv[0] == "-v":
        ...
        try:
            ...
            self.config = yaml.load(doc, Loader=EnvVarLoader)
            # self.config = EnvYAML(configFilename)
    ...
    def read_config(self):
        ...
        self.schemaTable = self.getconfig(
            "gpload:output:table", str, returnOriginal=True
        )
    ...
    def gpfdist_ssl(self, popenList):
        ...
        certificates_path = self.getconfig(
            "gpload:input:source:certificates_path", str, None
        )
            ...
            try:
                ...
                if platform.system() in [
                    ...
                    if os.environ.get("GPHOME_LOADERS"):
                        ...
                        srcfile = os.path.join(
                            os.environ.get("GPHOME"), "greenplum_path.sh"
                        )

                    if not (srcfile and os.path.exists(srcfile)):
                        self.log(
                            self.ERROR,
                            "cannot find greenplum environment "
    ...
    def setup_connection(self, recurse=0):
        ...
        if self.options.W:
            ...
            if self.options.password == None:
                self.options.password = self.getconfig("password", str, None)
            if self.options.password == None:
        ...
        try:
            ...
            if errorMessage.find("no password supplied") != -1:
                ...
                self.log(
                    ...
                    "the Greenplum Database running on port %i?"
    ...
    def read_columns(self):
        ...
        if columns != None:
            ...
            for d in columns:
                ...
                # for do_insert()
                self.from_columns.append(
                    [quote_ident(key), d[key].lower(), None, False]
                )
        else:
    ...
    def read_table_metadata(self):
        ...
        queryString = """select nt.nspname as table_schema,
         ...
         order by a.attnum """ % (
            quote_unident(self.table),
            quote_unident(self.schema),
        )
        ...
        while count < len(resultList):
            ...
            if ct == "bigserial":
                ...
                ct = "int4"
            name = row["column_name"].encode("utf-8")
    ...
    def get_staging_conditions_string(
        ...
        staging_cols_str = "-".join(
            [
                "%s-%s" % (quote(quote_unident(col[0])), quote(col[1]))
    ...
    def get_reuse_staging_table_query(self, encoding_conditions):
        ...
        sql = """SELECT oid::regclass
                 FROM pg_class
                 WHERE relname = 'staging_gpload_reusable_%s';""" % (
            encoding_conditions
        )
            ...
            if self.staging_table:
                ...
                if self.extSchemaName is None:
                    ...
                    sql = (
                        ...
                        % (quote_unident(self.extSchemaName), self.extTableName)
                    )
                result = self.db.query(sql.encode("utf-8")).getresult()
                if len(result) > 0:
                    self.extSchemaTable = self.get_ext_schematable(
                        quote_unident(self.extSchemaName), self.extTableName
                    )
                    self.log(
                        self.INFO,
                ...
                self.log(
                    ...
                    % self.get_ext_schematable(self.extSchemaName, self.extTableName),
                )
    ...
    def create_output_table(self):
        if not self.getconfig("gpload:preload:create_output_table", bool, default=None):
            return
        output_name = self.schemaTable
        # make sure we set the correct distribution policy
        # distcols = self.getconfig('gpload:output:match_columns', list)
        target_columns = []
        for column in self.into_columns:
            if column[2]:
                target_columns.append([quote_unident(column[0]), column[1]])
        sql = "CREATE TABLE IF NOT EXISTS %s " % (output_name)
        cols = ['"%s" %s' % (a[0], a[1]) for a in target_columns]
        sql += "(%s)" % ",".join(cols)
        # sql += " DISTRIBUTED BY (%s)" % ', '.join(distcols)
        self.log(self.LOG, sql)
    ...
    def do_update(self, fromname, index):
        ...
        update_condition = self.getconfig("gpload:output:update_condition", str, None)
        if update_condition:
            #
    ...
    def do_method_merge(self):
        ...
        where = self.map_stuff(
            "gpload:output:match_columns",
            lambda x, y: "(into_table.%s IS NULL OR CAST(into_table.%s AS varchar) = '')"
            % (x, x),
            0,
        )
    ...
    def do_method(self):
        ...
        if preload:
            ...
            self.staging_table = self.getconfig(
                "gpload:preload:staging_table", str, default=None
            )
        if self.error_table:
